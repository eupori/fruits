...
def get_settlement(first_datetime, last_datetime):
    ...
    etc = ["키트", "분석"]
    date_cols = []
    start_date = datetime.datetime.strptime("2022-06-23", "%Y-%m-%d")
    ...
    genetic_tests = GeneticTest.objects.all(start_datetime__year=start_date.year, start_datetime__month=start_date.month)
    
    exists_list = []
    person_data_list = []
    for genetic_test in genetic_tests:
        if genetic_test.coupon not in exists_list:
            exists_list.append(genetic_test.coupon)
        else:
            continue
        ...
        if kit_status == "cancel":
            ...
            billing_price = 6000
            genetic_test.is_active = False
        elif kit_status == "test_start_cancel":
            billing_price = 20000
            genetic_test.is_active = False
        ...
        if genetic_test.status == "cancel" and genetic_test.is_active:
            ...
            if genetic_test.end_datetime > genetic_test.start_datetime + relativedelta(days=14):
                ...
                if genetic_test.kit_status == "normal_kit_return":
                    ...
                    billing_price = 26000
                elif genetic_test.kit_status == "test_start_cancel":
                    billing_price = 20000
            genetic_test.is_active = False
            genetic_test.end_datetime = today
        ...
        for each in etc:
            ...
            person_data["이동전화번호"] = genetic_test.phone
            person_data["구분"] = each
            person_data["키트번호(최초)"] = first_kit
            person_data["키트번호(마지막 재채취)"] = last_kit
            person_data["가입일"] = genetic_test.customer.created_at.strftime("%Y.%m.%d")
    ...
    for item in logs:
        ...
        data.append(item.genetic_test.phone)
        data.append(item.genetic_test.test_product.get_connection_client_display() if genetic_test.test_product else "")
        data.append(item.genetic_test.test_product.product_code if genetic_test.test_product else "")
        data.append(item.genetic_test.test_product.product_name if genetic_test.test_product else "")
