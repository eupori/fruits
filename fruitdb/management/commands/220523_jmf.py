...
def get_settlement(first_datetime, last_datetime):
    ...
    #조건 확인 필요 : 현재는 이번달에 시작한 것만 나와있음, 원래는 이번달에 몇회차인지까지 나와야 해서 is_active가 True인 모든 상품에 대해서 해야 함 + 이번달 구독 해지 예정 고객도 포함해야 함(end_datatime으로 확인할 수 있는지 체크 필요)
    #is_active = False, end_datetime = 이번달,
    ...
    subs = JMFSubscription.objects.filter(start_datetime__year=today.year, start_datetime__month=today.month)

    person_data_list = []
    ################# 기타 수수료
    for genetic_test_product in genetic_test_products:
        in_round_subs = subs.filter(product=genetic_test_product,is_coupon_used=True, current_times__lte=12)
        out_round_subs = subs.filter(product=genetic_test_product,is_coupon_used=True, current_times__gt=12)
        ...
        # subs.filter(product=genetic_test_product, current_times=13, )
        #current_time == 13 & is_coupon_used = False
        #쿠폰 미사용(계약기간 이전, 만료)
        #쿠폰 사용(계약기간 이후)
        #키트 미사용
        #정상 키트 반품
        #파손 키트 반품
        #13회차때 쿠폰 미사용 시 3% 추가 청구건 개발 필요
        # kit_refund_list = subs.filter(product=genetic_test_product, is_coupon_used=False, current_times=13)
    ##########################
    ...
    logs = JMFSubscriptionPaymentLog.objects.exclude(jmf_subscription=None).exclude(jmf_subscription__user_name=None).filter(created_at__month=today.month)
    logs_qs = []
    max_kit_length = 0
    for item in logs:
        data = []
    ...
    phone_ids = list(set(logs_df["전화번호"].to_list()))
    df3 = pd.DataFrame(columns=columns)
    for phone in phone_ids:
        phone_df = logs_df.groupby(["전화번호"]).get_group(phone)
        phone_df = phone_df.sort_values(["날짜", "시간"], axis=0)
        df3 = df3.append(phone_df)
    ...
    wb.active
    for col in range(1, 19):
        wb["구독상품"].column_dimensions[get_column_letter(col)].width = 20
        wb["구독상품"].column_dimensions[get_column_letter(col)].height = 15
        if len(person_data_list) > 0:
            wb["단품상품"].column_dimensions[get_column_letter(col)].width = 20
