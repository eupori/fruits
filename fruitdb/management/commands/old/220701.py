...
class Settlement():
    ...
    def subscription_product(self):
        ...
        for product in products:
            logs = JMFSubscriptionPaymentLog.objects.filter(jmf_subscription__product=product, order_date__year=self.today.year, order_date__month=self.today.month)
            ...
            active_subs = JMFSubscription.objects.filter(is_active=True, product=product)
            ...
            data_dict["일시납 회원 결제 건수"] = len(deactive_subs)
            data_dict["일시납 회원 결제 금액"] = len(deactive_subs)
    ...
    def single_product(self):
        ...
        response = requests.request("GET", f"{url}/order/fruits?type=monthly")
        order_list = json.loads(response.text)["data"]

        response = requests.request("GET", f"{url}/paymentlog/fruits?type=monthly")
